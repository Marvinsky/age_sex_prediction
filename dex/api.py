...
def estimage_frame(frame):
    tensor_cuda_image = preprocess(frame)
    #age
    output_age = age_model(tensor_cuda_image)
    output_age = output_age.detach().numpy().squeeze()
    age = expected_age(output_age)
    #sex
    output_sex = gender_model(tensor_cuda_image)
    output_sex = output_sex.detach().numpy().squeeze()

    return age, output_sex[0], output_sex[1]
